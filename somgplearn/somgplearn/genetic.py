...
# --------------------------------------------------Edited code-----------------------------------
            ...
            for i in range(length_population):
                ...
                # sim = simplify(parse_expr(str(population[i]), transformations=transformations),locals=locals)
                sim = sympify(population[i])
                a.append(sim)
            ...
            for i in range(len1):
                for j in range(len(population[i])):               
                    print(simplify(population[i][j]))




            '''
                tost = listToString(population[i])
                a.append(tost)
                print(i, end=" test")
